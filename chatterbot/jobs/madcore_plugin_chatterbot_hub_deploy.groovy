pipelineJob('madcore.plugin.chatterbot.deploy') {
    parameters {
      stringParam('REPO_URL', 'https://github.com/madcore-ai/containers', '')
      stringParam('REPO_BRANCH', 'issue-124-chatterbot', '')
      stringParam('APP_NAME', 'chatterbot', '')
      stringParam('SERVICE_PORT', '9019', '')
      stringParam('DOCKERFILE_PATH', 'chatterbot', 'Specify path to docker file relative to root repo.')
      ...
      cps {
            ...
            node {
                ...
                stage ('Generate rc yaml') {
                  build job: 'madcore.plugin.chatterbot.render.template', parameters: [string(name: 'APP_NAME', value: params.APP_NAME)]
                }
                stage ('Kubernetes: create') {
                  build job: 'madcore.kubectl.create', parameters: [string(name: 'FILENAME', value: 'chatterbot/kub')]
                }
                ...
                stage ('add to ingress controller') {
                  if (params.MADCORE_INGRESS_FLAG == true) {
                    build job: 'madcore.plugin.ingress.add.service', parameters: [string(name: 'APP_NAME', value: params.APP_NAME), string(name: 'SERVICE_NAME', value: APP_SERVICE_NAME), string(name: 'SERVICE_PORT', value: params.SERVICE_PORT), string(name: 'SERVICE_NAMESPACE', value: params.APP_NAMESPACE) ]
                  }
