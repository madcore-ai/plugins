    ...
    definition {
	    cps {
            ...
            node {
                ...
                stage('Neo4j: create master') {
                    build job: 'madcore.kubectl.create', parameters: [string(name: 'FILENAME', value: 'neo4j/kub/neo4j-master-rc.yaml')]
                }
                stage('Neo4j: wait for master') {
                    build job: 'madcore.kubectl.wait.pod.up', parameters: [string(name: 'POD_NAME', value: 'neo4j-master')]
                }
                stage('Neo4j: create client') {
                    build job: 'madcore.kubectl.create', parameters: [string(name: 'FILENAME', value: 'neo4j/kub/neo4j-client-rc.yaml')]
                }
                stage('Neo4j: wait for client') {
                    build job: 'madcore.kubectl.wait.pod.up', parameters: [string(name: 'POD_NAME', value: 'neo4j-client')]
                }
                stage('Neo4j: create data') {
                    build job: 'madcore.kubectl.create', parameters: [string(name: 'FILENAME', value: 'neo4j/kub/neo4j-data-rc.yaml')]
                }
